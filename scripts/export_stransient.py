...
bus_df = (
    pd.read_csv(base / 'buses.csv')
    .rename(columns={'name': 'bus_id', 'v_nom': 'vn_kv'})
)
...
lines['type'] = 'AC_line'
lines[['branch_id','bus0','bus1','r_pu','x_pu','length','i_nom','type']].to_csv(out / 'stransient_branch.csv', index=False)
gens = pd.read_csv(base / 'generators.csv')
q_source = 'q_nom' if 'q_nom' in gens.columns else 'q_set' if 'q_set' in gens.columns else None
...
p_source = 'p_mw' if 'p_mw' in loads.columns else 'p_set' if 'p_set' in loads.columns else None
q_source = 'q_mvar' if 'q_mvar' in loads.columns else 'q_set' if 'q_set' in loads.columns else None
...
loads[['load_id', 'bus', 'p_mw', 'q_mvar']].to_csv(out / 'stransient_load.csv', index=False)
